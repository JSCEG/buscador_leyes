...
d=pymupdf.open('.local/lse-sync/LSE.pdf')
...
for p in d:
 for b in p.get_text('dict')['blocks']:
  for l in b.get('lines',[]):
   ...
   n=norm(s)
   if not n or re.fullmatch(r'\d+de56',n):continue
...
sha=hashlib.sha256(Path('.local/lse-sync/LSE.pdf').read_bytes()).hexdigest();sid='lse-'+sha[:12]
for a in arts:
...
source=dict(id=sid,title='Ley del Sector Eléctrico',lawId=arts[0]['ley_id'],sha256=sha,transport='remote-pdf',pdfUrl='/api/reader/'+sid,originalUrl='https://www.diputados.gob.mx/LeyesBiblio/pdf/LSE.pdf',pageCount=len(d),pages=[dict(number=p.number+1,width=p.rect.width,height=p.rect.height) for p in d])
...
assert len(mapped)==207 and not missing, 'El PDF no coincide con todos los fragmentos; no publicar.'
