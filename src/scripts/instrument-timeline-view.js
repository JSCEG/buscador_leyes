...
export function renderInstrumentTimeline(container, data, { onOpenLaw, onOpenArticle }) {
    ...
    container.innerHTML = `
        <section class="it-timeline" aria-labelledby="instrument-timeline-title">
            ...
            <p class="it-intro">Publicaciones y documentos vinculados en el acervo. Cada uno conserva su texto y alcance.</p>
            <ol class="it-events" aria-label="Documentos por fecha" tabindex="0">
                ${entries.map(entry => `
                    <li class="it-event${entry.current ? ' it-current' : ''}" data-timeline-id="${escapeHtml(entry.id)}">
                        <div class="it-date"><span class="it-point" aria-hidden="true"></span>${entry.date ? `<time datetime="${entry.date}">${dateText(entry.date)}</time>` : '<span>Fecha no identificada</span>'}<small>${entry.date ? escapeHtml(entry.dateLabel) : 'Sin ubicación cronológica'}</small></div>
                        <div class="it-card" ${entry.current ? 'aria-current="true"' : ''}>
                            <div class="it-tags"><span>${escapeHtml(entry.type)}</span>${entry.current ? '<strong>En consulta</strong>' : ''}</div>
                            ...
                            <div class="it-actions">
                                ${entry.current ? '<span class="it-open-current">Documento abierto</span>' : `<a class="it-open" data-timeline-key="${escapeHtml(entry.key)}" href="#${entry.kind === 'instrument' ? 'ley' : 'art'}-${encodeURIComponent(entry.id)}" aria-label="Abrir ${escapeHtml(entry.title)}">Abrir documento <span aria-hidden="true">→</span></a>`}
            ...
            <p class="it-note">${entries.length === 1 ? 'Aún no hay otros documentos vinculados a este instrumento en el acervo.' : 'El orden por fecha no implica que todos los documentos modifiquen al instrumento. Esta vista no es un texto consolidado.'}${unavailableCount ? ' Hay referencias a registros que todavía no están disponibles en el acervo.' : ''}</p>
        </section>`;
