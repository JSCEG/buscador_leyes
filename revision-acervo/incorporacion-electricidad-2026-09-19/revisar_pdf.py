...
for s in json.loads((R/'fuentes.json').read_text(encoding='utf8')):
 ...
 if name in ('DACG-ACCESO-REDES','UNIDADES-INSPECCION'):
  # El HTML coloca al final una nota al pie de la página 251. El documento
  # continúa hasta el tercer folio del Anexo 5: no usar esa nota como cierre.
  marker=norm('ANEXO 5 A/025/2023 3/3' if name=='DACG-ACCESO-REDES' else 'Ser un inspector honorable, con valores éticos.')
  hits=[i+1 for i,t in enumerate(txt)if marker in norm(t)]
  assert len(hits)==1
  end=hits[0]
 else:
 ...
 samples={start,end}
 if name=='DACG-ACCESO-REDES':samples.update([end-4,end-2])
