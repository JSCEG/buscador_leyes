...
## 16. Tres instrumentos de autoconsumo — estado actual al 18 de septiembre de 2026

Se incorporaron uno por uno AUTOCONSUMO-0.7-20 (requisitos del permiso de generación interconectado, DOF 6 de agosto de 2025), FORMATO-AUTOCONSUMO (acuerdo que publica el formato, DOF 7 de octubre de 2025) y VENTANILLA-AUTOCONSUMO (acuerdo y lineamientos de la Ventanilla Única, DOF 8 de mayo de 2026). Catálogo confirmado: **41 instrumentos, 3,300 fragmentos y 904 entradas de estructura**. Los 38 instrumentos, 3,237 fragmentos y 831 temas anteriores se compararon campo por campo, incluidas las fechas de creación, y permanecen sin cambios.

Las tres altas suman 63 fragmentos (60 de las fuentes y tres notas editoriales) y 73 entradas de estructura. Requisitos tiene 9 fragmentos/6 temas; formato, 6/13; Ventanilla, 48/54. Se separaron resolutivos y transitorios. La Ventanilla conserva ocho capítulos, 36 numerales de segundo nivel con todos sus subnumerales e incisos y tres anexos completos. El formato y su instructivo permanecen en un solo fragmento; sus campos numerados no se convirtieron en artículos.

Fuentes SIDOF: 5764827, 5769388 y 5786923. Se cotejaron exactamente los 457 bloques HTML y la estructura de 36 tablas con 1,068 celdas, incluidos colspan/rowspan. Las ocho imágenes del formato se inspeccionaron como casillas vacías y se representan con □. El cotejo léxico independiente cubre 50 páginas PDF de las tres publicaciones (3 + 8 + 39), con revisión visual complementaria. La única diferencia de vocabulario controlada son 17 viñetas circulares que el HTML oficial de la Ventanilla expresa como letra «o»; se conserva esa fuente. La comparación léxica por sí sola no certifica orden ni maquetación: se complementó con bloques, celdas y revisión visual.

Cada alta se ejecutó en una transacción con controles de duplicados, cobertura, texto y FTS. Los respaldos completos previos y posteriores confirman que cada carga coincide exactamente con el payload revisado. Los textos mantienen su publicación y ámbito propios; no se presentan como versiones consolidadas ni se infiere su vigencia. Las tres notas editoriales enlazan los documentos relacionados sin confundir los requisitos, el formato y los lineamientos. No se modificó el parser genérico ni la tabla pendiente de relaciones.

En la app pública se comprobaron las 63 tarjetas sin anidamientos, el resolutivo de requisitos, el formato completo y los tres anexos en el modal. Las tablas y casillas coinciden con la fuente revisada. El anexo A conserva sus 22 tablas y 861 celdas también en una ventana de 390 × 844, sin desbordar el documento. La búsqueda global «autoconsumo» devuelve 149 coincidencias en 18 documentos. Lint y comprobación sintáctica de los scripts CJS correctos.

Evidencia y scripts: `revision-acervo/incorporacion-autoconsumo-2026-09-18/`. Informe `INCORPORACION.html`/`.md`, `COTEJO-FUENTES.json`, `COTEJO-PDF.json`, `VERIFICACION.json`, `VERIFICACION-NAVEGADOR.json`, manifiesto de SQL y registro de las tres transacciones; fuentes, payloads y respaldos en `antes-validado/` y `despues-verificado/`. Los SQL ya fueron aplicados y no deben ejecutarse de nuevo. Los perfiles son específicos para estas tres fuentes y rechazan cambios de huella o estructura.

Se regeneró el inventario en HTML/Markdown/JSON/CSV: **101 referencias pendientes, 32 cargadas, una parcial, 13 antecedentes, cuatro filas de proyectos, diez portales y tres publicaciones no localizadas**, sobre las mismas 164 referencias y 179 filas del radar v4.17, corte 14 de septiembre de 2026. RAD-067, RAD-068 y RAD-034 están cubiertas por las tres altas. Hay 37 instrumentos representados en la sección 12 y cuatro fuera. `catalogo-antes-autoconsumo.json` conserva los 38 instrumentos previos y `catalogo-actual.json`, los 41 actuales. Siguiente bloque propuesto: cogeneración (DACG y formatos), con cotejo individual antes de cada carga.
