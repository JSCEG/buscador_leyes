...
d=pymupdf.open('.local/lpte-sync/LPTE.pdf')
...
for p in d:
 for b in p.get_text('dict')['blocks']:
  for l in b.get('lines',[]):
   ...
   n=norm(s)
   if not n or re.fullmatch(r'\d+de23',n):continue
...
sha=hashlib.sha256(Path('.local/lpte-sync/LPTE.pdf').read_bytes()).hexdigest();sid='lpte-'+sha[:12]
for a in arts:
...
source=dict(id=sid,title='Ley de Planeación y Transición Energética',lawId=arts[0]['ley_id'],sha256=sha,transport='remote-pdf',pdfUrl='/api/reader/'+sid,originalUrl='https://www.diputados.gob.mx/LeyesBiblio/pdf/LPTE.pdf',pageCount=len(d),pages=[dict(number=p.number+1,width=p.rect.width,height=p.rect.height) for p in d])
...
assert len(mapped)==113 and not missing, 'El PDF no coincide con todos los fragmentos; no publicar.'
