...
.dark-mode .atema-nav-btn.active .atema-nav-label { color: #f5f5f5; }
    ...
    `).join("");return`
        ...
        </div>`}function kr(u){const g=Math.max(...u.menciones.map(t=>t.valor));return`
    ...
    `).join("");u.innerHTML=`
        <div style="max-width:800px;margin:0 auto;padding-bottom:3rem">
            ...
            ${g}
            ${t}
        </div>`,(o=u.querySelector("#analisis-back-btn"))==null||o.addEventListener("click",()=>{document.dispatchEvent(new CustomEvent("analisis:goHome"))});const e=u.querySelectorAll(".atema-nav-btn"),r=u.querySelectorAll(".atema-card-wrapper");e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(f=>f.classList.remove("active")),n.classList.add("active");const i=n.dataset.temaId;r.forEach(f=>{f.classList.toggle("hidden",f.id!=="wrapper-"+i)});const c=u.querySelector("#wrapper-"+i);c&&pt(c)})}),ut.forEach(n=>Cr(u,n))}function $r(){var Ct,St,$t,Bt,_t;const u=document.getElementById("search-input"),g=document.getElementById("results-container"),t=document.getElementById("law-detail-container"),e=document.getElementById("stats-minimal"),r=document.getElementById("hero-section"),o=document.getElementById("main-container"),n=document.getElementById("quick-filters"),i=document.getElementById("detail-modal"),c=document.getElementById("modal-panel"),f=document.getElementById("modal-content"),k=document.getElementById("modal-title"),L=document.getElementById("modal-ley"),A=document.getElementById("close-modal"),M=document.getElementById("copy-btn"),P=document.getElementById("loading-indicator"),z=document.getElementById("nav-inicio"),T=document.getElementById("nav-leyes"),R=document.getElementById("mobile-menu-btn"),j=document.getElementById("mobile-menu-overlay"),N=document.getElementById("mobile-menu-drawer"),J=document.getElementById("close-mobile-menu"),s=document.getElementById("mobile-nav-inicio"),ae=document.getElementById("mobile-nav-leyes"),de="app-dark-mode";let ue=localStorage.getItem(de)==="true";function ce(a){if(ue=a,localStorage.setItem(de,a),document.documentElement.classList.toggle("dark-mode",a),!document.getElementById("global-dark-style")){const p=document.createElement("style");p.id="global-dark-style",p.innerHTML=`
            ...
            `,document.head.appendChild(p)}const l=document.querySelectorAll("#darkmode-icon-moon, #mobile-darkmode-moon"),d=document.querySelectorAll("#darkmode-icon-sun, #mobile-darkmode-sun"),m=document.getElementById("mobile-darkmode-label");l.forEach(p=>p.classList.toggle("hidden",a)),d.forEach(p=>p.classList.toggle("hidden",!a)),m&&(m.textContent=a?"Modo claro":"Modo oscuro")}ue&&ce(!0),(Ct=document.getElementById("darkmode-toggle"))==null||Ct.addEventListener("click",()=>ce(!ue)),(St=document.getElementById("mobile-darkmode-toggle"))==null||St.addEventListener("click",()=>ce(!ue));function oe(a){!N||!j||(a?(j.classList.remove("hidden"),j.offsetWidth,j.classList.remove("opacity-0"),N.classList.remove("translate-x-full"),document.body.style.overflow="hidden"):(j.classList.add("opacity-0"),N.classList.add("translate-x-full"),document.body.style.overflow="",setTimeout(()=>{j.classList.add("hidden")},300)))}R&&R.addEventListener("click",()=>oe(!0)),J&&J.addEventListener("click",()=>oe(!1)),j&&j.addEventListener("click",()=>oe(!1)),s&&s.addEventListener("click",a=>{a.preventDefault(),Ie(),oe(!1)}),ae&&ae.addEventListener("click",a=>{a.preventDefault(),Pe(),oe(!1)});let U=[],q=[];window.addEventListener("search-ready",a=>{const{totalLeyes:l,totalArticulos:d,summaries:m}=a.detail;U=m,e&&(e.innerHTML=`
        ...
        `}function Me(a){var ve,nt,It,At,Tt,jt,Pt;if(!t)return;ye(),gt(),q=mr(a.titulo);const l=[...new Set(q.map(S=>S.capitulo_nombre).filter(Boolean))];[...new Set(q.map(S=>S.titulo_nombre).filter(Boolean))];const d=q.filter(S=>S.articulo_label.toLowerCase().includes("transitorio")).length;g.classList.add("hidden"),r.classList.add("hidden"),n.classList.add("hidden"),e.classList.add("hidden"),(ve=document.getElementById("analisis-container"))==null||ve.classList.add("hidden","opacity-0"),t.classList.remove("hidden"),setTimeout(()=>t.classList.remove("opacity-0"),50),$(`#ley-${encodeURIComponent(a.id)}`);let m=100,p=localStorage.getItem("reader-theme")||"light";t.innerHTML=`
...
`)[0]}</div>`:""}
        ...
        `,document.body.appendChild(w);const v=q.map((S,F)=>{const ne=S.articulo_label.match(/\d+/),te=ne?ne[0]:F+1,Fe=!!Re(S.id);return`<button class="toc-art-btn text-[11px] font-medium rounded-lg py-2 px-1 border transition-all text-center relative
                ...
                Art.${te}
                ${Fe?'<span class="absolute top-0.5 right-0.5 w-1.5 h-1.5 bg-amber-400 rounded-full"></span>':""}
            </button>`}).join(""),b=q.map(S=>{const F=!!Re(S.id),ne=Oe(S.id),te=S.titulo_nombre?`<span class="text-gray-400 ml-1 font-normal">· ${S.titulo_nombre}</span>`:"",Fe=S.texto?S.texto.replace(/\s+/g," ").substring(0,120).trim()+(S.texto.length>120?"...":""):"";return`<button class="toc-art-btn w-full flex flex-col gap-2 px-3 py-2.5 rounded-xl text-left transition-all hover:bg-guinda/5 group/item
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="flex-shrink-0 flex items-center gap-1">
                        ${F?'<span class="w-1.5 h-1.5 bg-amber-400 rounded-full" title="Tiene nota"></span>':""}
                ...
                ${Fe?`<span class="text-[11px] text-gray-500 leading-tight line-clamp-2">${Fe}</span>`:""}
        ...
        `,document.body.appendChild(h);const x=h.querySelector("#toc-tab-grid"),E=h.querySelector("#toc-tab-list"),y=h.querySelector("#toc-content-grid"),C=h.querySelector("#toc-content-list"),_=h.querySelector("#toc-search"),H=["bg-guinda","text-white","shadow-sm"],W=["text-gray-500","hover:text-guinda","hover:bg-guinda/5"];x.addEventListener("click",()=>{x.classList.add(...H),x.classList.remove(...W),E.classList.remove(...H),E.classList.add(...W),y.classList.remove("hidden"),C.classList.add("hidden"),_&&(_.value=""),y.querySelectorAll(".toc-art-btn").forEach(S=>S.style.display="")}),E.addEventListener("click",()=>{E.classList.add(...H),E.classList.remove(...W),x.classList.remove(...H),x.classList.add(...W),C.classList.remove("hidden"),y.classList.add("hidden"),_&&(_.value=""),C.querySelectorAll(".toc-art-btn").forEach(S=>S.style.display="")}),_&&(_.addEventListener("input",S=>{const F=S.target.value.toLowerCase().trim();(C.classList.contains("hidden")?y:C).querySelectorAll(".toc-art-btn").forEach(te=>{var it;const Fe=!F||((it=te.title)==null?void 0:it.toLowerCase().includes(F))||te.textContent.toLowerCase().includes(F);te.style.display=Fe?"":"none"})}),_.addEventListener("click",S=>S.stopPropagation()));let ee=!1;const Se=S=>{ee=S,S?(h.classList.remove("translate-y-full"),document.body.style.overflow="hidden"):(h.classList.add("translate-y-full"),document.body.style.overflow="")};w.addEventListener("click",()=>Se(!ee)),(nt=h.querySelector("#toc-close-btn"))==null||nt.addEventListener("click",()=>Se(!1)),h.querySelectorAll(".toc-art-btn").forEach(S=>{S.addEventListener("click",()=>{Se(!1),fe(S.dataset.id)})}),ze(q.slice(0,20),""),setTimeout(()=>{ft(q)},100);const he=document.getElementById("law-articles-list"),$e=document.getElementById("btn-font-increase"),Be=document.getElementById("btn-font-decrease"),be=document.getElementById("font-size-display"),De=document.querySelectorAll(".theme-btn");document.getElementById("law-header-area");const We=S=>{if(p=S,localStorage.setItem("reader-theme",S),document.body.className=`bg-${S} text-gray-900 font-body min-h-screen flex flex-col antialiased transition-colors duration-300`,De.forEach(F=>{F.classList.remove("ring-2","ring-guinda","ring-offset-1"),F.dataset.theme===S&&F.classList.add("ring-2","ring-guinda","ring-offset-1")}),document.querySelectorAll(".mob-theme-btn").forEach(F=>{F.classList.remove("border-guinda","text-guinda"),F.classList.add("border-transparent"),F.dataset.theme===S&&(F.classList.remove("border-transparent"),F.classList.add("border-guinda"),S!=="dark"&&F.classList.add("text-guinda"))}),!document.getElementById("reader-themes-style")){const F=document.createElement("style");F.id="reader-themes-style",F.innerHTML=`
                    /* Sepia Mode */
                ...
                `,document.head.appendChild(F)}};We(p);const _e=()=>{he&&(he.style.fontSize=`${m}%`),document.querySelectorAll("#font-size-display, #mob-font-display").forEach(S=>{S.innerText=`${m}%`})};$e&&$e.addEventListener("click",()=>{m<250&&(m+=10,_e())}),Be&&Be.addEventListener("click",()=>{m>80&&(m-=10,_e())}),be&&(be.addEventListener("click",()=>{m=100,_e()}),be.style.cursor="pointer",be.title="Restablecer al 100%"),De.forEach(S=>{S.addEventListener("click",F=>{We(F.target.dataset.theme)})});const D=document.getElementById("mobile-reading-toggle"),se=document.getElementById("mobile-reading-sheet"),le=document.getElementById("mobile-reading-overlay"),Je=S=>{se==null||se.classList.toggle("translate-y-full",!S),le==null||le.classList.toggle("hidden",!S)};D==null||D.addEventListener("click",()=>Je(!0)),le==null||le.addEventListener("click",()=>Je(!1)),(It=document.getElementById("mob-font-decrease"))==null||It.addEventListener("click",()=>{m>80&&(m-=10,_e())}),(At=document.getElementById("mob-font-increase"))==null||At.addEventListener("click",()=>{m<250&&(m+=10,_e())}),document.querySelectorAll(".mob-theme-btn").forEach(S=>{S.addEventListener("click",()=>{We(S.dataset.theme),Je(!1)})});const Qe=document.getElementById("law-share-btn"),re=document.getElementById("law-share-menu");document.getElementById("law-share-text-btn"),document.getElementById("law-share-link-btn"),Qe&&re&&(Qe.addEventListener("click",S=>{S.stopPropagation(),re.classList.toggle("hidden")}),document.addEventListener("click",function S(F){F.target.closest("#law-share-wrapper")||(re.classList.add("hidden"),document.removeEventListener("click",S))})),Object.entries({"law-share-whatsapp-btn":()=>Ue(a,"whatsapp"),"law-share-telegram-btn":()=>Ue(a,"telegram"),"law-share-twitter-btn":()=>Ue(a,"twitter"),"law-share-email-btn":()=>Ue(a,"email"),"law-share-link-btn":()=>{const S=`${location.origin}${location.pathname}#ley-${encodeURIComponent(a.id)}`;navigator.clipboard.writeText(S).then(()=>Z("¡Enlace copiado!","🔗","bg-blue-600"))}}).forEach(([S,F])=>{var ne;(ne=document.getElementById(S))==null||ne.addEventListener("click",()=>{re==null||re.classList.add("hidden"),F()})}),(Tt=document.getElementById("print-btn"))==null||Tt.addEventListener("click",()=>window.print()),(jt=document.getElementById("crumb-inicio"))==null||jt.addEventListener("click",()=>Ie()),(Pt=document.getElementById("crumb-categoria"))==null||Pt.addEventListener("click",()=>Pe()),document.querySelectorAll(".theme-filter-btn").forEach(S=>{S.addEventListener("click",F=>{const ne=F.target.dataset.theme,te=document.getElementById("law-search-input");te&&(te.value=ne,te.dispatchEvent(new Event("input")))})}),document.getElementById("law-search-input").addEventListener("input",S=>{const F=S.target.value.toLowerCase().trim();let ne=q;F.length>2&&(ne=q.filter(te=>te.texto.toLowerCase().includes(F)||te.articulo_label.toLowerCase().includes(F)||te.titulo_nombre&&te.titulo_nombre.toLowerCase().includes(F)||te.capitulo_nombre&&te.capitulo_nombre.toLowerCase().includes(F))),ze(ne.slice(0,50),F)}),document.getElementById("export-csv-btn").addEventListener("click",()=>{Gt(q,`${a.titulo}.csv`)})}function ft(a){const l=document.getElementById("law-structure-chart");if(!l)return;if(!window.d3){l.innerHTML='<div class="flex items-center justify-center h-full text-xs text-gray-400">Cargando visualización...</div>',setTimeout(()=>ft(a),1e3);return}if(l.innerHTML="",!a||a.length===0){l.innerHTML='<div class="flex items-center justify-center h-full text-xs text-gray-400">No hay datos para visualizar</div>';return}const d={};a.forEach(C=>{let _=C.titulo_nombre||C.capitulo_nombre||"General";_=_.replace(/^TÍTULO\s+/i,"").replace(/^CAPÍTULO\s+/i,""),_=_.replace(/^[IVXLCDM]+\.?\s*-?\s*/,""),_=_.replace(/^(PRIMERO|SEGUNDO|TERCERO|CUARTO|QUINTO|SEXTO|SÉPTIMO|OCTAVO|NOVENO|DÉCIMO)\.?\s*-?\s*/i,""),_=_.trim(),_||(_="General"),_.length>25&&(_=_.substring(0,25)+"..."),d[_]=(d[_]||0)+1});const m=Object.entries(d).map(([C,_])=>({name:C,value:_})).sort((C,_)=>_.value-C.value).slice(0,5);if(m.length===0){l.innerHTML='<div class="flex items-center justify-center h-full text-xs text-gray-400">Datos insuficientes</div>';return}const p={top:10,right:30,bottom:20,left:220},w=l.clientWidth,b=Math.max(l.clientHeight,m.length*35+p.top+p.bottom);d3.select(l).select("svg").remove();const h=d3.select(l).append("svg").attr("width","100%").attr("height",b).attr("viewBox",[0,0,w,b]).attr("style","max-width: 100%; height: auto; font: 11px sans-serif;"),x=d3.scaleLinear().domain([0,d3.max(m,C=>C.value)]).range([p.left,w-p.right]),E=d3.scaleBand().domain(m.map(C=>C.name)).rangeRound([p.top,b-p.bottom]).padding(.3);d3.selectAll(".d3-tooltip").remove();const y=d3.select("body").append("div").attr("class","d3-tooltip absolute bg-gray-900/90 backdrop-blur text-white text-[10px] rounded-lg py-1.5 px-3 pointer-events-none opacity-0 transition-opacity z-50 shadow-xl border border-gray-700").style("display","none");h.append("g").attr("fill","#9B2247").selectAll("rect").data(m).join("rect").attr("x",x(0)).attr("y",C=>E(C.name)).attr("width",C=>Math.max(0,x(C.value)-x(0))).attr("height",E.bandwidth()).attr("rx",4).on("mouseover",(C,_)=>{d3.select(C.target).attr("fill","#7A1C39"),y.style("opacity","1").style("display","block").text(`${_.name}: ${_.value} artículos`)}).on("mousemove",C=>{y.style("left",C.pageX+10+"px").style("top",C.pageY-10+"px")}).on("mouseout",C=>{d3.select(C.target).attr("fill","#9B2247"),y.style("opacity","0").style("display","none")}),h.append("g").attr("fill","black").attr("text-anchor","start").attr("font-size","10px").selectAll("text").data(m).join("text").attr("x",C=>x(C.value)+4).attr("y",C=>E(C.name)+E.bandwidth()/2).attr("dy","0.35em").text(C=>C.value),h.append("g").call(d3.axisLeft(E).tickSize(0)).attr("transform",`translate(${p.left},0)`).call(C=>C.select(".domain").remove()).call(C=>C.selectAll("text").attr("fill","#4B5563").attr("font-weight","500").style("text-anchor","end").attr("dx","-6"))}function ze(a,l){const d=document.getElementById("law-articles-list");if(d){if(a.length===0){d.innerHTML='<div class="text-center py-8 text-gray-400 text-sm">No se encontraron artículos que coincidan con la búsqueda.</div>';return}ke=a,d.innerHTML=a.map(m=>{const p=l?Ve(m.texto,l):m.texto.substring(0,300)+"...",w=!!Re(m.id),v=Oe(m.id)?'<svg class="w-3.5 h-3.5 text-guinda" fill="currentColor" viewBox="0 0 24 24"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>':'<svg class="w-3.5 h-3.5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>',b=ie.includes(m.id),h=b?"text-guinda":ie.length>=2?"text-gray-100":"text-gray-300 hover:text-guinda",x=b?"bg-guinda/10":"";return`
...
`),w=new Blob([p],{type:"text/csv;charset=utf-8;"}),v=document.createElement("a");if(v.download!==void 0){const b=URL.createObjectURL(w);v.setAttribute("href",b),v.setAttribute("download",l),v.style.visibility="hidden",document.body.appendChild(v),v.click(),document.body.removeChild(v)}}function Wt(a){const l=Ke().filter(d=>d!==a);l.unshift(a),localStorage.setItem("search-history",JSON.stringify(l.slice(0,10)))}function Ke(){return JSON.parse(localStorage.getItem("search-history")||"[]")}n&&n.addEventListener("click",a=>{a.target.tagName==="BUTTON"&&(u.value=a.target.textContent,u.dispatchEvent(new Event("input")))});function Ne(a){return a.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"")}function ht(a,l){const d=Ne(a),m=Ne(l),p=d.indexOf(m);return p===-1?we(a):we(a.slice(0,p))+`<mark class="bg-guinda/10 text-guinda font-semibold not-italic">${we(a.slice(p,p+l.length))}</mark>`+we(a.slice(p+l.length))}function we(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}if(u){let d=function(){return Array.from(a.querySelectorAll("[data-navigable]"))},m=function(b){const h=d();h.forEach((x,E)=>{x.classList.toggle("bg-gray-50",E===b),x.setAttribute("aria-selected",E===b?"true":"false")}),l=b,h[b]&&h[b].scrollIntoView({block:"nearest"})},p=function(){a.classList.add("hidden"),l=-1},w=function(b){var x;l=-1;const h=[];if(b){const E=U.filter(H=>Ne(H.titulo).includes(Ne(b))).slice(0,4);E.length>0&&h.push({label:"Leyes",items:E.map(H=>({html:`
                                ...
                                <span class="text-sm text-gray-700 font-medium truncate">${ht(H.titulo,b)}</span>`,attrs:`data-navigable data-law-title="${we(H.titulo)}" class="px-4 py-3 hover:bg-gray-50 cursor-pointer flex items-center gap-3 transition-colors suggestion-law"`}))});const y=gr(),C=Ne(b),_=[];for(const H of y){if(_.length>=4)break;const W=H.articulo_label||"",ee=H.titulo_nombre||"",Se=H.capitulo_nombre||"",he=[W,ee,Se].find($e=>$e&&Ne($e).includes(C));he&&_.push({art:H,matchField:he})}_.length>0&&h.push({label:"Artículos",items:_.map(({art:H,matchField:W})=>({html:`
...
${d}`,v=`${a.titulo} — Marco Legal Energético SENER`,b={whatsapp:`https://wa.me/?text=${encodeURIComponent(w)}`,telegram:`https://t.me/share/url?url=${encodeURIComponent(d)}&text=${encodeURIComponent(m)}`,twitter:`https://twitter.com/intent/tweet?text=${encodeURIComponent(v)}&url=${encodeURIComponent(d)}`,email:`mailto:?subject=${encodeURIComponent(m)}&body=${encodeURIComponent(w)}`};b[l]&&window.open(b[l],"_blank")}function kt(){var b,h;$(null),ye(),je(),r.classList.add("hidden"),n.classList.add("hidden"),e.classList.add("hidden"),t&&t.classList.add("hidden","opacity-0"),(b=document.getElementById("analisis-container"))==null||b.classList.add("hidden","opacity-0"),o.classList.remove("justify-center","pt-24"),o.classList.add("pt-8"),g.classList.remove("hidden"),setTimeout(()=>g.classList.remove("opacity-0"),50);const a=document.getElementById("search-filters");if(a&&a.remove(),U.length===0){g.innerHTML='<div class="text-center py-16 text-gray-400">Cargando datos...</div>';return}const l=U.reduce((x,E)=>x+E.articulos,0),d=U.filter(x=>x.titulo.toLowerCase().startsWith("ley")),m=U.filter(x=>x.titulo.toLowerCase().startsWith("reglamento")),p=U.filter(x=>!x.titulo.toLowerCase().startsWith("ley")&&!x.titulo.toLowerCase().startsWith("reglamento")),w=[...U].sort((x,E)=>E.articulos-x.articulos),v=((h=w[0])==null?void 0:h.articulos)||1;g.innerHTML=`
            <div class="w-full mb-8">
            ...
            <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm mb-6">
                ...
                <div class="space-y-3">
                    ${w.map(x=>{const E=x.titulo.toLowerCase().startsWith("ley"),y=x.titulo.toLowerCase().startsWith("reglamento"),C=E?"#9B2247":y?"#1E5B4F":"#A57F2C",_=Math.round(x.articulos/v*100);return`
                        <div class="flex items-center gap-3 cursor-pointer group stat-law-row" data-titulo="${x.titulo.replace(/"/g,"&quot;")}">
                            ...
                            <div class="flex-1 h-5 bg-gray-50 rounded-full overflow-hidden">
                                <div class="h-full rounded-full transition-all duration-500" style="width:${_}%; background:${C};"></div>
                            </div>
        ...
        `,document.querySelectorAll(".stat-law-row").forEach(x=>{x.addEventListener("click",()=>{const E=U.find(y=>y.titulo===x.dataset.titulo);E&&Me(E)})})}function Ae(){var h,x,E;if(!g)return;let a=O;if(G.type!=="all"&&(a=a.filter(y=>G.type==="ley"?y.ley_origen.toLowerCase().includes("ley"):G.type==="reglamento"?y.ley_origen.toLowerCase().includes("reglamento"):!y.ley_origen.toLowerCase().includes("ley")&&!y.ley_origen.toLowerCase().includes("reglamento"))),G.law!=="all"&&(a=a.filter(y=>y.ley_origen===G.law)),G.artNum){const y=parseInt(G.artNum);a=a.filter(C=>{const _=C.articulo_label.match(/\d+/);return _&&parseInt(_[0])===y})}const l=a,d=B,m=document.getElementById("search-filters");if(m&&m.remove(),O.length>0){const y=document.createElement("div");y.id="search-filters",y.className="flex flex-col items-center gap-2 mb-6 animate-fade-in-up";const C=[...new Set(O.map(W=>W.ley_origen))].sort();y.innerHTML=`
            ...
            `,g.parentNode.insertBefore(y,g),y.querySelectorAll(".filter-btn").forEach(W=>{W.addEventListener("click",ee=>{G.type=ee.target.dataset.type,V=1,Ae()})});const _=document.getElementById("law-filter-select");_&&_.addEventListener("change",W=>{G.law=W.target.value,V=1,Ae()});const H=document.getElementById("art-number-filter");if(H){let W;H.addEventListener("input",ee=>{clearTimeout(W),W=setTimeout(()=>{G.artNum=ee.target.value.trim(),V=1,Ae()},400)})}(h=document.getElementById("clear-all-filters"))==null||h.addEventListener("click",()=>{G={type:"all",law:"all",artNum:""},V=1,Ae()})}if(l.length===0){const y=G.type!=="all"||G.law!=="all";g.innerHTML=`
                <div class="text-center py-16 px-4">
                    ...
                    <div class="flex flex-wrap gap-2 justify-center mb-4">
                        ${["Transmisión","Generación","CENACE","Distribución","Tarifas","Permisos"].map(_=>`<button class="empty-suggestion px-4 py-1.5 bg-gray-50 border border-gray-100 rounded-full text-xs text-gray-500 hover:bg-guinda/5 hover:border-guinda/30 hover:text-guinda transition-all">${_}</button>`).join("")}
                    </div>
            ...
            <div class="group relative bg-white border border-transparent hover:border-gray-100 rounded-xl p-5 hover:shadow-lg hover:shadow-gray-100/50 transition-all duration-300 cursor-pointer result-item" data-id="${y.id}">
                ...
                <h3 class="text-lg font-serif font-bold text-gray-800 mb-2 group-hover:text-guinda transition-colors">${_}</h3>
                <p class="text-sm text-gray-500 font-light leading-relaxed line-clamp-3">${C}</p>
            </div>
            `}).join(""),X(l.length,"results-container",Ae),document.querySelectorAll(".result-item").forEach(y=>{y.addEventListener("click",C=>{C.target.closest(".bookmark-card-btn")||fe(y.dataset.id)})}),document.querySelectorAll(".bookmark-card-btn").forEach(y=>{y.addEventListener("click",C=>{C.stopPropagation(),Ze(y.dataset.id),Ae()})})}function fe(a){const l=Te(a);if(!l)return;L.textContent=l.ley_origen,k.textContent=l.articulo_label,L.onclick=()=>{const D=U.find(se=>se.titulo===l.ley_origen);D&&(Ge(),setTimeout(()=>Me(D),310))};let d=l.texto.replace(/\r\n/g,`
`).replace(/\n\s*\n/g,`
...
`).map(D=>`<p class="mb-4">${m(D)}</p>`).join("")}
        ...
        `;const h=ke.findIndex(D=>D.id===a),x=ke.length,E=document.getElementById("modal-prev-btn"),y=document.getElementById("modal-next-btn"),C=document.getElementById("modal-nav-counter");E&&(E.disabled=h<=0,E.onclick=()=>{h>0&&fe(ke[h-1].id)}),y&&(y.disabled=h<0||h>=x-1,y.onclick=()=>{h<x-1&&fe(ke[h+1].id)}),C&&(C.textContent=h>=0?`${h+1}/${x}`:"");const _=document.getElementById("modal-bookmark-btn");if(_){const D=Oe(a);_.innerHTML=D?'<svg class="w-5 h-5 text-guinda" fill="currentColor" viewBox="0 0 24 24"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>':'<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>',_.onclick=()=>{Ze(a),fe(a)}}const H=document.getElementById("copy-btn");H&&(H.onclick=()=>{navigator.clipboard.writeText(f.innerText).then(()=>{Z("¡Texto copiado!","📋")})});const W=document.getElementById("share-btn"),ee=document.getElementById("share-menu");document.getElementById("share-text-btn"),document.getElementById("share-image-btn"),W&&ee&&(W.onclick=D=>{D.stopPropagation(),ee.classList.toggle("hidden")},document.addEventListener("click",function D(se){se.target.closest("#share-menu-wrapper")||(ee.classList.add("hidden"),document.removeEventListener("click",D))}));const Se=Re(a);f.innerHTML+=`
