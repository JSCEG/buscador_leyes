...
CONFIG={s['name']:(s['titulo'].rstrip('.'),METADATA[s['name']]['tipo'],[s['familia'], s['pieza'], 'Trámites eléctricos'])for s in SOURCES}
...
def note(source):
 ...
 text+='Publicaciones relacionadas, conservadas por separado y sin consolidar sus textos:\n\n'
 for s in SOURCES:
  if s['familia']!=source['familia']:continue
 ...
 text+='\nSe conserva la numeración de cada publicación. Las tablas, instrucciones y casillas forman parte de los fragmentos correspondientes. La consulta del original debe acompañarse de sus aclaraciones y modificaciones; no se presenta un texto consolidado ni se certifica su vigencia. Fuentes cotejadas el 19 de septiembre de 2026.\n'
...
def build(name):
 ...
 assert next(c for c in load(ROOT/'COTEJO-PDF.json')if c['instrumento']==name)['cotejo_aprobado']
