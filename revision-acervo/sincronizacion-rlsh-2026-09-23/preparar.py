...
d=pymupdf.open('.local/rlsh-sync/RLSH.pdf')
norm=lambda s:re.sub(r'\s+','',s).translate(str.maketrans({'“':'"','”':'"'}))
lines=[];text=''
for p in d:
 for b in p.get_text('dict')['blocks']:
  for l in b.get('lines',[]):
   ...
   n=norm(s)
   if not n or re.fullmatch(r'\d+de88',n):continue
...
sha=hashlib.sha256(Path('.local/rlsh-sync/RLSH.pdf').read_bytes()).hexdigest();sid='rlsh-'+sha[:12]
for a in arts:
 match_text=a['contenido']
 if a['identificador']=='Preámbulo':
  heading,match_text=match_text.split('\n',1)
  assert heading=='DECRETO por el que se expide el Reglamento de la Ley del Sector Hidrocarburos.'
  # El DOF repite este título al inicio. Su texto está también al final del
  # mismo preámbulo y queda resaltado allí en la edición de Diputados.
...
source=dict(id=sid,title='Reglamento de la Ley del Sector Hidrocarburos',lawId=arts[0]['ley_id'],sha256=sha,transport='remote-pdf',pdfUrl='/api/reader/'+sid,originalUrl='https://www.diputados.gob.mx/LeyesBiblio/regley/Reg_LSH.pdf',pageCount=len(d),pages=[dict(number=p.number+1,width=p.rect.width,height=p.rect.height) for p in d])
...
assert len(mapped)==371 and not missing, 'Correspondencia inesperada; no publicar.'
